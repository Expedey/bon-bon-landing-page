  ...
  useEffect(() => {
    ...
    allWords.forEach((word, index) => {
      ...
      tl.to(
        ...
        index === 0 ? 0 : (index - 1) / (totalWords - 1) // Skip first word, start from second
      );
      ...
      tl.to(
        cursorRef.current,
        {
          ...
          y: () => {
            ...
            const textRect = textRef.current?.getBoundingClientRect();
            if (textRect) {
              return wordRect.top - textRect.top - 20;
            }
            return 0;
          },
          duration: 0.1,
        ...
        index === 0 ? 0 : (index - 1) / (totalWords - 1) + 0.05 // Skip first word, start from second
      );
    });
    ...
    tl.to(
      ...
      (totalWords - 2) / (totalWords - 1) + 0.1 // Start right after the last word animation (accounting for skipped first word)
    );
    ...
    >
      <div className="max-w-[1400px] relative mx-auto px-4 w-full flex justify-center items-center min-h-screen">
        <div className="relative">
          ...
          <div
            ref={cursorRef}
            className="absolute w-[7px] h-9 lg:h-14 xl:h-20 bg-[#6a4afd] shadow-[-20px_0px_94px_40px_#6a4afd66]"
            style={{
              left: "0px",
              top: "24px",
